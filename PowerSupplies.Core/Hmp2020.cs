...
public sealed class Hmp2020
{
    public string HmpInfo
    {
        get
        {
            WriteLine("*IDN?");
    ...
    {
        _event.WaitOne();
        var current = MeasureCurrentRequest(channel);
        _event.Set();
        return current;
    }
    ...
    public void SetOutput(bool output)
    {
        ...
        SetOutput(1, output ? 1 : 0);
        SetOutput(2, output ? 1 : 0);
    ...
    private void SetVoltageCurrent(int channel, double voltage, double current)
    {
        double power = channel == 1 ? 160.0 : 80.0;
